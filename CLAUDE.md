...
- **MockTraceAgent** - CLI application for command-line trace inspection
...
dotnet build MockTraceAgent.csproj

# Run the CLI application (default port 8126)
dotnet run --project MockTraceAgent.csproj

# Run with custom port
dotnet run --project MockTraceAgent.csproj -- --port 8080

# Show trace/span counts (requires deserialization)
dotnet run --project MockTraceAgent.csproj -- --show-counts

# Save raw payloads to files
dotnet run --project MockTraceAgent.csproj -- --save RawBytes

# Save payloads as JSON
dotnet run --project MockTraceAgent.csproj -- --save ConvertToJson

# Save both raw and JSON
dotnet run --project MockTraceAgent.csproj -- --save All

# Filter by URL (default: /traces)
dotnet run --project MockTraceAgent.csproj -- --url-filter /v0.4/traces
```
...
### MockTraceAgent (CLI Application)
